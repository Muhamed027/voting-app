<div x-data={isOpen:false} x-show="isOpen" x-cloak @keydown.escape.window="isOpen = false" x-init=" window.livewire.on('IdeaWasUpdated', () => {
     isOpen = false
 })"
    @custom-show-edit-idea-modal.window="isOpen=true" class=" fixed  z-10 inset-0 overflow-y-auto"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center  dark:text-white dark:bg-slate-900 min-h-screen">
        <div x-show="isOpen" x-transition.opacity.1000ms
            class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
        <div x-show="isOpen" x-transition.origin.bottom.duration.1000ms
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form wire:submit.prevent="updateIdea" action="#" method="POST" class="space-y-4 px-4 py-6">
                    ...
                    <div class="flex items-center justify-between space-x-3">
                        ...
                        <button type="submit"
                            class="flex items-center justify-center w-1/2 h-11 text-xs bg-blue-500 text-white font-semibold rounded-xl border border-blue-700 hover:bg-blue-hover transition duration-150 ease-in px-6 py-3">
